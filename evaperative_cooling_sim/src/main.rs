...
impl Ramp {
    pub fn update_step(&mut self, step: i64, timestep: f64) {
...
impl<'a> System<'a> for Ramp {
    ...
    fn run(&mut self, (mut gaussian_beam): Self::SystemData) {
        let mut tmp_power = 0.0;
        for beam in (&mut gaussian_beam).join() {
            beam.power = self.initial_power*(2.0_f64).powf(-1.0*self.rate*self.time);
            tmp_power = beam.power
        }
        Ramp::update_step(self, self.step, self.delta);
        Ramp::output(self, 2.0*tmp_power);
    }
}

fn main() {

    let power_file_location: String = "../data/power_distro00.txt".to_string();
    ...
    builder = builder.with(
        file::new::<atom::Position, Text>("../data/pos_distro00.txt".to_string(), 10),
        "",
        &[],
    );
    builder = builder.with(
        file::new::<atom::Velocity, Text>("../data/vel_distro00.txt".to_string(), 10),
        "",
        &[],
    );
    ...
    let mut dispatcher = builder
        .with( Ramp { step: 1, time: 0.0, rate: 4.0/3.0, delta: dt, initial_power: power, file: power_file_location}, "ramp", &[])
        .build();
