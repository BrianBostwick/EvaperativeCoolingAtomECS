...
fn main() {
    ...
    let mut loc1 = "/Users/brianbostwick/Mirror/Cambridge/QMBP_Lab/DipoleTrap_EvapCooling/data/pos_trapfreqtest19.txt";
    // let mut loc2 = "/Users/brianbostwick/Mirror/Cambridge/QMBP_Lab/DipoleTrap_EvapCooling/data/range.txt";
    // let mut loc3 = "/Users/brianbostwick/Mirror/Cambridge/QMBP_Lab/DipoleTrap_EvapCooling/data/rangeforce.txt";
    ...
    //
    // builder = builder.with(
    //     file::new_with_filter::<lib::atom::Force, SerdeJson, Atom>(loc3.to_string(), 10),
    //     "",
    //     &[INTEGRATE_VELOCITY_SYSTEM_NAME],
    // );

    //Beam paramters
    let power = 7.0; //initial power of the ramp
    ...
    world
        .create_entity()
        .with(Position {
            pos: Vector3::new(1.0e-6, 1.0e-6, 1.0e-6),
        })
        .with(atom::Velocity {
            vel: Vector3::new(0.0, 0.0, 0.0),
        })
        .with(dipole::Polarizability::calculate_for(
                 wavelength, 461e-9, 30.5e6,
             ))
        .with(Atom)
        .with(NewlyCreated)
        .with(Force::new())
