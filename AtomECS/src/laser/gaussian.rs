...
#[derive(Deserialize, Serialize, Clone, Copy, Lerp, Debug)]
...
pub fn get_gaussian_beam_intensity_gradient(
	...
	let spot_size_squared =
		2.0*beam.e_radius.powf(2.0) * (1. + (z / beam.rayleigh_range).powf(2.0)); //changes this from 2.0*beam.e_radius.powf(2.0) * (1. + (z / beam.rayleigh_range).powf(2.0));
	let vector = -4.*(reference_frame.x_vector * x + reference_frame.y_vector * y) // 4. -> 1. for first prefactor.
